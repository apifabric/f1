...
resourcename="f1"  # lower case, only
resourcegroup="f1_rg"
...
# git clone https://github.com/apilogicserver/f1.git
# cd f1
...
az group create --name f1_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group f1_rg --sku S1 --is-linux
az container create --resource-group f1_rg --name f1 --image apilogicserver/f1:latest --dns-name-label f1 --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//f1.westus.azurecontainer.io:5656'
